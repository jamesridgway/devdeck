    ...
    def test_initialize_sets_icon_and_resets_name_index(self):
        ...
        with mock_context(name_list_control) as ctx:
            ...
            assert_rendered(ctx, TestingUtils.get_filename(
                '../devdeck/assets/font-awesome/users.png'))

    def test_pressed_iterates_initials(self):
        settings = {'names': ['Sarah Mcgrath',
                              'Eduardo Sanders', 'Ellis Banks']}
        ...
        with mock_context(name_list_control) as ctx:
            ...
            assert_rendered(ctx, Renderer().text('SM').font_size(
                256).center_vertically().center_horizontally().end())
            assert_that(name_list_control.name_index).is_equal_to(1)

            name_list_control.pressed()
            assert_rendered(ctx, Renderer().text('ES').font_size(
                256).center_vertically().center_horizontally().end())
            assert_that(name_list_control.name_index).is_equal_to(2)

            name_list_control.pressed()
            assert_rendered(ctx, Renderer().text('EB').font_size(
                256).center_vertically().center_horizontally().end())
